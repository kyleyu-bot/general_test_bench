...
struct PdoLogRecord {
    ...
    // ── JIP algorithm parameters (stamped from CommandState each cycle) ────────
    float    jip_inertia            = 0.f;
    float    jip_hardstop_pos_upper = 0.f;
    float    jip_hardstop_pos_lower = 0.f;
    float    jip_margin             = 0.f;
};
...
inline constexpr const char* PDO_LOG_CSV_HEADER =
    ...
    "jip_inertia,jip_hardstop_pos_upper,jip_hardstop_pos_lower,jip_margin";
