    ...
    std::chrono::steady_clock::time_point reset_end)
{
        ...
        (const SystemStatus& status, const LoopStats& stats) -> SystemCommand
    {
        ...
        main_cmd.dahl_coulomb_force            = cmd.main_dahl_coulomb_force;
        main_cmd.linear_damping_comp_enable    = cmd.main_linear_damping_enable;
        main_cmd.linear_damping_comp_gain      = cmd.main_linear_damping_gain;
        ...
        dut_cmd.dahl_coulomb_force            = cmd.dut_dahl_coulomb_force;
        dut_cmd.linear_damping_comp_enable    = cmd.dut_linear_damping_enable;
        dut_cmd.linear_damping_comp_gain      = cmd.dut_linear_damping_gain;
        ...
        auto fill_tx = [&](const DriveStatus& ds, bool is_main) {
            if (is_main) {
                ...
                rec.main_tx_imu_accel_y       = ds.imu_accel_y;
                rec.main_tx_imu_accel_z       = ds.imu_accel_z;
                rec.main_tx_imu_gyro_x        = ds.imu_gyro_x;
                rec.main_tx_imu_gyro_y        = ds.imu_gyro_y;
                rec.main_tx_imu_gyro_z        = ds.imu_gyro_z;
                ...
                rec.dut_tx_imu_accel_y        = ds.imu_accel_y;
                rec.dut_tx_imu_accel_z        = ds.imu_accel_z;
                rec.dut_tx_imu_gyro_x         = ds.imu_gyro_x;
                rec.dut_tx_imu_gyro_y         = ds.imu_gyro_y;
                rec.dut_tx_imu_gyro_z         = ds.imu_gyro_z;
...
std::string DualNovantaTestbench::serializeToCsvRow(const dyno::PdoLogRecord& r)
{
    ...
    o << r.main_tx_statusword        << ',' << static_cast<int>(r.main_tx_mode_display)  << ','
      ...
      << r.main_tx_imu_accel_y       << ','
      << r.main_tx_imu_accel_z       << ','
      << r.main_tx_imu_gyro_x        << ','
      << r.main_tx_imu_gyro_y        << ','
      << r.main_tx_imu_gyro_z        << ','
    ...
    o << r.dut_tx_statusword         << ',' << static_cast<int>(r.dut_tx_mode_display)   << ','
      ...
      << r.dut_tx_imu_accel_y        << ','
      << r.dut_tx_imu_accel_z        << ','
      << r.dut_tx_imu_gyro_x         << ','
      << r.dut_tx_imu_gyro_y         << ','
      << r.dut_tx_imu_gyro_z         << ','
    ...
    const DriveGains& gains)
{
    ...
    if (it != status.by_slave.end() && it->second.has_value()) {
        ...
        j["imu_accel_y"]        = ds.imu_accel_y;
        j["imu_accel_z"]        = ds.imu_accel_z;
        j["imu_gyro_x"]         = ds.imu_gyro_x;
        j["imu_gyro_y"]         = ds.imu_gyro_y;
        j["imu_gyro_z"]         = ds.imu_gyro_z;
